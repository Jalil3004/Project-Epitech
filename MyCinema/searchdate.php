...
<nav class="navbar navbar-expand-lg navbar-dark bg-black">
    ...
    <input type="date" name="s1" placeholder="date_begin">
...
if(isset($_GET['s']) AND !empty($_GET['s']) OR isset($_GET['s1']) AND !empty($_GET['s1'])){
    ...
    $req= mysqli_query($con, 'SELECT date_begin,title FROM movie_schedule INNER JOIN movie ON id_movie=movie.id WHERE title LIKE "%'.$recherche.'%" AND date_begin LIKE "%'.$recherche1.'%"'); 
